...
from ProG.Model.model import GraphSAGE
...
def train(model, data, device, train_loader, optimizer, loss_all):
    model.train()
...
def main(args):
    ...
    for epoch in range(args.n_epochs):
        ...
        acc_all.append(acc)
        t0 = time.time()
        loss = train(model, data, device, train_loader, optimizer, loss_all)
